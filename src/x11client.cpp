    ...
    , m_focusOutTimer(nullptr)
{
    ...
    m_syncRequest.calledSyncRequest = false;
...
void X11Client::killProcess(bool ask, xcb_timestamp_t timestamp)
{
    ...
    if (!ask) {
        ...
        const QFileInfo buildDirBinary{QDir{QCoreApplication::applicationDirPath()}, QStringLiteral("deepin-kwin_killer_helper")};
...
void X11Client::getSyncCounter()
{
    ...
    if (counter != XCB_NONE) {
        ...
        if (!ready_for_painting && m_syncRequest.calledSyncRequest) {
            sendSyncRequest();
        }
    }
}
...
{
    m_syncRequest.calledSyncRequest = true;
