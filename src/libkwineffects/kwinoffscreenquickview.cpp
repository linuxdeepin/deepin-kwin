...
void OffscreenQuickScene::setSource(const QUrl &source, const QVariantMap &initialProperties)
{
    ...
    std::unique_ptr<QObject> qmlObject(d->qmlComponent->create());
    ...
    for (auto it = initialProperties.begin(); it != initialProperties.end(); ++it) {
        item->setProperty(it.key().toStdString().c_str(), it.value());
    }
