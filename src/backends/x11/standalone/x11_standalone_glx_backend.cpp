...
const FBConfigInfo &GlxBackend::infoForVisual(xcb_visualid_t visual)
{
    ...
    if (!direct) {
        qCCritical(KWIN_X11STANDALONE).nospace() << "Could not find a picture format for visual 0x" << std::hex << visual;
        return info;
    }
    ...
    if (count < 1) {
        qCCritical(KWIN_X11STANDALONE).nospace() << "Could not find a framebuffer configuration for visual 0x" << std::hex << visual;
        return info;
    }
    ...
    if (info.fbconfig) {
        ...
        qCDebug(KWIN_X11STANDALONE).nospace() << "Using FBConfig 0x" << std::hex << fbc_id << " for visual 0x" << std::hex << visual_id;
    }
