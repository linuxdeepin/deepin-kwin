...
void PipeWireStream::newStreamParams()
{
    ...
    const int cursorSize = Cursor::self()->themeSize() * m_cursor.scale;
...
bool PipeWireStream::createStream()
{
    ...
    if (m_cursor.mode == KWayland::Server::ScreencastV1Interface::Embedded) {
        connect(Cursor::self(), &Cursor::posChanged, this, [this] {
            if (m_cursor.lastFrameTexture) {
                m_repainting = true;
                recordFrame(m_cursor.lastFrameTexture.data(), QRegion{m_cursor.lastRect} | cursorGeometry(Cursor::self()));
                m_repainting = false;
            }
        });
    }
...
QImage PipeWireStream::softwareCursor() const
{
    return kwinApp()->platform()->softwareCursor();
}

QPoint PipeWireStream::softwareCursorHotspot() const
{
    return kwinApp()->platform()->softwareCursorHotspot();
}
...
void PipeWireStream::recordFrame(GLTexture *frameTexture, const QRegion &damagedRegion)
{
    ...
    if (data || spa_data[0].type == SPA_DATA_MemFd) {
        ...
        if (m_cursor.mode == KWayland::Server::ScreencastV1Interface::Embedded && m_cursor.viewport.contains(cursor->pos())) {
            ...
            if (!m_cursor.texture || m_cursor.lastKey != softwareCursor().cacheKey())
                m_cursor.texture.reset(new GLTexture(softwareCursor()));

            m_cursor.texture->setYInverted(false);
            m_cursor.texture->bind();
            const auto cursorRect = cursorGeometry(cursor);
            mvp.translate(cursorRect.left(), r.height() - cursorRect.top() - softwareCursor().height() * m_cursor.scale);
            shader->setUniform(GLShader::ModelViewProjectionMatrix, mvp);
...
void PipeWireStream::sendCursorData(Cursor *cursor, spa_meta_cursor *spa_meta_cursor)
{
    ...
    spa_meta_cursor->hotspot.x = softwareCursorHotspot().x() * m_cursor.scale;
    spa_meta_cursor->hotspot.y = softwareCursorHotspot().y() * m_cursor.scale;
    spa_meta_cursor->bitmap_offset = 0;

    const QImage image = softwareCursor();
...
void PipeWireStream::setCursorMode(KWayland::Server::ScreencastV1Interface::CursorMode mode, qreal scale, const QRect &viewport)
{
    m_cursor.mode = mode;
