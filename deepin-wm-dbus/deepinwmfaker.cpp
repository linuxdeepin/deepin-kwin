...
{
    QString uri = getWorkspaceBackgroundForMonitor(index, strMonitorName);
    if (uri.isEmpty()) {
        uri = _gsettings_dde_appearance->get(GsettingsBackgroundUri).toStringList().value(index - 1);
        if (index == 1) {
            if(!QFileInfo(uri).isFile()) {
        ...
        const QString &workSpaceBackgroundUri = uri;
...
void DeepinWMFaker::SetDecorationTheme(const QString &type, const QString &name)
{
    ...
    m_kwinConfig->group("deepin-chameleon").writeEntry("theme", QString("%1/%2").arg(type).arg(name));
